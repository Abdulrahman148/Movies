...
@AndroidEntryPoint
class PopularFragment : Fragment(), MovieClickListener, MovieDetailsClickListener {
    ...
    override fun onMovieClicked(movie: Result) {
        ...
        movieViewModel.updatePopularMovie(movie)
    ...
    override fun onMovieDetailsClicked(movie: Result) {
        val action = PopularFragmentDirections.actionPopularFragmentToMovieDetailsFragment(movie.title, movie.overview, movie.isFavourites.toString() , movie.release_date , movie.original_language, movie.vote_average.toString(), movie.poster_path, movie.backdrop_path)
